    ...
    with tabs[i]:
        ...
        # =================================================================
            ...
            if df.empty:
                ...
                # Renombrar canales de Consumo: el primero a 'Consumo SB' y el consumo masivo a 'Consumo PU'
                consumo_count = 0

                def _renombrar_canal_proy(val):
                    nonlocal consumo_count
                    v_str = str(val).strip()
                    if "CONSUMO" in v_str.upper():
                        consumo_count += 1
                        return "Consumo SB" if consumo_count == 1 else "Consumo PU"
                    return v_str

                df_proy[mes_col] = df_proy[mes_col].apply(_renombrar_canal_proy)
                ...
                canales_principales = ["Consumo SB", "Consumo PU", "Consumo", "Consumo Masivo", "Farma", "Terceros"]
                ...
                st.markdown("""
                * **Consumo SB / Consumo PU y Farma**: Representan los motores principales del negocio, alcanzando un desempeño relevante sobre sus metas proyectadas.
                * **Terceros**: Presenta un desempeño rezagado con un avance parcial respecto a su meta global.
                """)

                # =================================================================
                # SECCIÓN TABLA Y GRÁFICO MEJORADO DE OC Y PROYECCIÓN COMPRA
                # =================================================================
                st.divider()
                ...
                col_k2.metric("🚚 Total Proyección Salida", formato_moneda(tot_proy_salida))
                ...
                with col_oc_tabla:
                    ...
                    st.dataframe(
                        ...
                        column_config={
                            ...
                            "Monto OC": st.column_config.NumberColumn("Monto OC", format="$%,.0f"),
                            "FR": st.column_config.NumberColumn("FR", format="%d%%"),
                            "Proyección salida": st.column_config.NumberColumn("Proyección salida", format="$%,.0f"),
                            "OC extra": st.column_config.NumberColumn("OC extra", format="$%,.0f"),
                        },
                        hide_index=True,
                        use_container_width=True,
                    )
                    ...
                    if target_r is not None and target_c is not None:
                        ...
                        for _, row_data in df_rows.iterrows():
                            ...
                            if (
                                ...
                                # Diferenciación dinámica: el primer consumo es Consumo SB y el consumo masivo/segundo es Consumo PU
                                if any(r.get("División") == "CONSUMO SB" for r in filas_procesadas):
                                    div_val = "CONSUMO PU"
                                else:
                                    div_val = "CONSUMO SB"
                            elif (
        ...
        # =================================================================
            ...
            if is_sb and col_div and col_oc:
                ...
                kf3.metric("🛒 OC Consumo SB", str(oc_consumo))
                kf4.metric("🛍️ Monto Consumo SB", formato_moneda(monto_consumo))
            ...
            # =================================================================
            # MÉTRICAS DE OC Y MONTO TOTAL PARA PU
            # =================================================================
            ...
            if col_semana:
                ...
                if sem_actual is not None:
                    ...
                    if is_pu:
                        ...
                        with col_r2:
                            fig_g = crear_reloj_gauge("CONSUMO PU", fr_tot, "#0070f3")
                            st.plotly_chart(
                                fig_g,
                                use_container_width=True,
                        ...
                        with col_r1:
                            fig_g_cons = crear_reloj_gauge(
                                "CONSUMO SB", fr_consumo_monto, "#f97316"
                            )
                            st.plotly_chart(
                                fig_g_cons,
                                use_container_width=True,
